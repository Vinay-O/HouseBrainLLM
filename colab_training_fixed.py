#!/usr/bin/env python3
"""
HouseBrain LLM Colab Training Script - FIXED VERSION

This script contains all the fixes we discovered during training:
- Proper gradient setup with prepare_model_for_kbit_training
- Correct dataset structure handling
- Memory optimizations for Colab
- Fixed LoRA configuration
- Proper data collator setup
...
@dataclass
class FixedFineTuningConfig:
    """Configuration for fine-tuning - FIXED VERSION"""
    model_name: str = "deepseek-ai/deepseek-coder-6.7b-base"
    dataset_path: str = "housebrain_dataset_v5_350k"
    output_dir: str = "models/housebrain-colab-trained"
...
class FixedHouseBrainDataset:
    """Dataset class for HouseBrain fine-tuning - FIXED VERSION"""
    
    def __init__(self, config: FixedFineTuningConfig):
        self.config = config
        self.tokenizer = None
        self.dataset = None
        
    def _load_dataset(self, split: str) -> List[Dict[str, Any]]:
        """Load dataset from JSON files - FIXED PATH HANDLING"""
    ...
    def _convert_to_house_input(self, sample: Dict[str, Any]) -> HouseInput:
        """Convert dataset sample to HouseInput - FIXED FORMAT HANDLING"""
        try:
    ...
    def prepare_dataset(self, max_samples: Optional[int] = None) -> Dataset:
        """Prepare dataset: returns un-tokenized prompt/response pairs."""
...
class FixedHouseBrainFineTuner:
    """Main fine-tuning class - FIXED VERSION"""
    ...
    def setup_model_and_tokenizer(self):
        """Setup model and tokenizer with proper quantization - FIXED"""
        ...
        # Load model with quantization - FIXED
        if self.config.use_4bit and self.config.device != "cpu":
            ...
            # FIXED: Prepare model for k-bit training
        ...
        # FIXED: Ensure model is in training mode and gradients are enabled
        self.model.train()
    ...
    def _setup_lora(self):
        """Setup LoRA configuration - FIXED"""
    ...
    def setup_trainer(self, train_dataset: Dataset, eval_dataset: Optional[Dataset] = None):
        """Setup trainer with proper configuration - FIXED"""
        ...
        training_args = TrainingArguments(
            ...
            # FIXED: Temporarily disable gradient checkpointing to avoid conflicts
    ...
    def train(self, max_samples: Optional[int] = None):
        ...
        try:
            ...
            # Prepare dataset
            dataset_generator = FixedHouseBrainDataset(self.config)
            ...
            if max_samples:
                print(f"📊 Training on {max_samples} samples (memory optimized)...")
            else:
...
def main():
    ...
    print("🏗️ HouseBrain LLM Training - FIXED VERSION")
    print("=" * 60)
    
    # Configuration
    config = FixedFineTuningConfig(
        model_name="deepseek-ai/deepseek-coder-6.7b-base",
        dataset_path="housebrain_dataset_v5_350k",
